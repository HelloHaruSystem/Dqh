...
/// <summary>Draws static map decorations (signs, etc.) from their real textures in <c>Assets/Props</c>, looked up by decoration id.</summary>
internal sealed class PropRenderer : IDisposable
{
    private readonly Dictionary<string, Texture2D> _textures = [];
    private readonly string _directory = Path.Combine(AppContext.BaseDirectory, "Assets", "Props");

    public void Draw(IReadOnlyList<DecorationData> decorations, Camera camera, Viewport viewport)
    {
        foreach (var decoration in decorations)
        {
            var screenColumn = decoration.Column - camera.StartColumn;
            var screenRow = decoration.Row - camera.StartRow;
            if (screenColumn < 0 || screenColumn >= camera.VisibleColumns || screenRow < 0 || screenRow >= camera.VisibleRows)
            {
                continue;
            }

            var texture = GetTexture(decoration.Id);
